...
class AttentionExtractor:
    ...
    def detect_robust(
        ...
        Strategy 1: GroundingDINO (Standard)
        Strategy 2: SAM 2 Grid Segmentation + LLM Selection
        """
        import os
        import cv2
        
        def is_valid(bbox, img_w, img_h):
            ...
            if area_ratio > max_area:
                return False, f"Area {area_ratio:.2f} > {max_area}"
            ...
            if ar < aspect_ratio_range[0] or ar > aspect_ratio_range[1]:
                return False, f"Aspect Ratio {ar:.2f} outside {aspect_ratio_range}"
        ...
        print(f"[AttentionExtractor] Robust Detection: Strategy 1 - Standard (prompts={prompts})", file=sys.stderr)
        for prompt in prompts:
            ...
            for det in detections:
                valid, reason = is_valid(det["bbox"], img_w, img_h)
                if valid:
                    valid_dets.append(det)
                    
            if valid_dets:
                print(f"[AttentionExtractor] Found {len(valid_dets)} valid objects with prompt '{prompt}'", file=sys.stderr)
                return valid_dets[:max_objects]

        # Strategy 2: SAM 2 + Planner Selection
        print("[AttentionExtractor] Robust Detection: Strategy 2 - SAM 2 Grid + Planner Selection", file=sys.stderr)
        
        # 1. Run SAM 2 with high point density
        ...
        # Filter masks by size/aspect ratio before showing to planner
        # to reduce clutter
        candidates = []
        for i, mask in enumerate(masks):
            ...
            if w < 5 or h < 5: continue # Too small
            
            bbox_norm = [x1/img_w, y1/img_h, x2/img_w, y2/img_h]
            valid, _ = is_valid(bbox_norm, img_w, img_h)
            
            if valid:
                candidates.append(mask)
        
        if not candidates:
            print("[AttentionExtractor] No valid candidates after filtering.", file=sys.stderr)
            return []
            
        # 2. Visualize candidates with IDs
        debug_path = "debug_sam2_candidates.png"
        self.visualize_masks(image_path, candidates, output_path=debug_path, draw_ids=True)
        
        # 3. Ask Planner
        from .planner import ActionPlanner
        # Use default config (gemini-2.5-flash-lite or similar)
        # Note: Depending on environment, API key might be needed.
        # Assuming environment is set up.
        try:
            planner = ActionPlanner()
            
            # Use the first prompt as the description
            target_desc = prompts[0]
            
            print(f"[AttentionExtractor] Asking planner to identify '{target_desc}' from {len(candidates)} candidates...", file=sys.stderr)
            selected_id = planner.select_items(debug_path, target_desc, len(candidates))
            
            if selected_id is not None:
                print(f"[AttentionExtractor] Planner selected ID {selected_id}", file=sys.stderr)
                
                # Convert selected mask to detection format
                mask = candidates[selected_id]
                y_indices, x_indices = np.where(mask > 0)
                x1, x2 = np.min(x_indices), np.max(x_indices)
                y1, y2 = np.min(y_indices), np.max(y_indices)
                
                # Standardize bbox format: [x_min, y_min, x_max, y_max] normalized
                bbox = [
                    float(x1) / img_w,
                    float(y1) / img_h,
                    float(x2) / img_w,
                ...
                return [{
                    "bbox": bbox,
                    "label": target_desc,
                    "score": 1.0, # High confidence since manually selected
                    "source": "sam2_planner"
                }]
            else:
                print("[AttentionExtractor] Planner could not identify the object.", file=sys.stderr)
                
        except Exception as e:
             print(f"[AttentionExtractor] Planner error: {e}", file=sys.stderr)
